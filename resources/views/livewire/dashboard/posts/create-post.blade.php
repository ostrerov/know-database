    ...
    <script type="module">
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            tinymce.init({
                ...
                plugins: 'autoresize preview importcss searchreplace autolink autosave directionality code visualblocks visualchars fullscreen image link media advanced-template codesample table charmap pagebreak nonbreaking anchor insertdatetime advlist lists wordcount help charmap quickbars emoticons',
...
{{--<div>
    <form class="row g-3" wire:submit="save" id="createPostForm">
        <div class="col-12">
        ...
        <div class="col-12">
            <label for="coverURL" class="form-label">Обложка поста</label>
            <input class="form-control" wire:model="coverURL" type="file" id="coverURL">
            @error('coverURL')
            <div id="coverURLFeedback" class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <livewire:dashboard.posts.create-category :category="$category" />
        <div class="col-sm-12 col-md-4 d-grid gap-0 h-25">
            <button class="btn btn-outline-secondary text-center d-none d-sm-none d-md-block" data-bs-toggle="modal" data-bs-target="#createCategoryModal" style="margin-top: 30px" type="button">{{ __('Создать категорию') }}</button>
            <button class="btn btn-outline-secondary text-center d-sm-block d-md-none" data-bs-toggle="modal" data-bs-target="#createCategoryModal" type="button">{{ __('Создать категорию') }}</button>
        </div>
